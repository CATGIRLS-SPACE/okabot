...
export async function PrivacyGuardCheckLinks(message: Message) {
    ...
    // it's very sloppy
    if ((message.content.includes('open.spotify') || message.content.includes('spotify.link')) && (message.content.includes('?si=') || message.content.includes('&si='))) {
        // link will 99% of the time be open.spotify.com/track/abcdef?si=fjsdahfjkdshfjk
        // so just split at the ? and roll
        const parts = message.content.split(' ');
        let new_content = '';
        for (const part of parts) {
            if ((part.includes('open.spotify') || part.includes('spotify.link')) && (part.includes('?si=') || part.includes('&si='))) new_content += ' ' + part.split('?')[0];
